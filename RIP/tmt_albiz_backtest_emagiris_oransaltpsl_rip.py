'''
- EMA 3 Close > EMA 8 close ise long gir
- EMA 3 Close < EMA 8 close ise short gir
- İşleme EMA ile girer, işlemden belirli oranda TP veya SL ile çıkar.
'''
...
oranTakeProfit = 0.002 # percent
oranStopLoss = 0.01 # percent
...
get_historical_data_symbol("Future", symbol, "1 May, 2022", "4 May, 2022", interval)
...
for i in range(df.shape[0]):
    ...
    # LONG İŞLEM
        ...
        if (start == True) and (position == "") and (long_signal == True):
            ...
            islemFiyati = open_price
            islemBuyuklugu = cuzdan * kaldirac  
            islemFee = islemBuyuklugu * feeOrani
            toplamFee += islemFee       
            stop_loss_price = islemFiyati * (1 - oranStopLoss)
            take_profit_price = islemFiyati * (1 + oranTakeProfit)
        ...
        # Long İşlem Kapat
        if (start == True) and (position == "Long") and ((high_price >= take_profit_price) or (short_signal == True)):
            #(stop_loss_price >= low_price)):
            
            if ((short_signal == True)):
                hedefFiyati = ema_buy_price
            elif (high_price >= take_profit_price):
                hedefFiyati = take_profit_price
    ...
    # SHORT İŞLEM
        ...
        # Short İşlem Kapat
        if (start == True) and (position == "Short") and ((high_price >= take_profit_price) or  (long_signal == True)):
            if (stop_loss_price >= low_price):
                hedefFiyati = ema_buy_price
            elif (high_price >= take_profit_price):
                hedefFiyati = take_profit_price
