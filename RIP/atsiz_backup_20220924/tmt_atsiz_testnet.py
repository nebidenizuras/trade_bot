...
from user_api_key import TESTNET_API_KEY, TESTNET_API_SECRET_KEY
...
client = Client(testnet=True, api_key = TESTNET_API_KEY,api_secret=TESTNET_API_SECRET_KEY)

# Get Current Price of Symbol
def get_current_price_of_symbol(coin_symbol):
...
def get_wallet(market='Future'):
    ...
    for i in balances:
        if float(i['free'])*get_current_price_of_symbol(f"{i['asset']}USDT")>10:
            result.append(f"{i['asset']}USDT")

    return result

# Get Wallet
def get_symbol_free_quantity_in_wallet(symbol: str):
    quantity = float(0)
...
pricePrecision = get_precision(symbol)
...
while(True):
    ...
    ### LONG İŞLEM ###
        ...
        if (start == True) and (position == "") and (long_signal == True) and ((current_price >= longGirisFiyat >= previous_price) or (current_price <= longGirisFiyat <= previous_price)):
            ...
            current_price = get_current_price_of_symbol(symbol)
            order_quantity = cuzdan / current_price
            order_quantity = float(round(order_quantity, get_tick_step_size(symbol)))
            client.futures_create_order(type=ORDER_TYPE_MARKET, 
                                        side= SIDE_BUY, 
                                        symbol=symbol, 
                                        positionSide = 'LONG',
